...
function Simplex:Get(vec: Vector): number
	...
	if vec.Size <= 3 then
		...
		local x = frequency * (vec[1] or 0)
		local y = frequency * (vec[2] or 0)
		local z = frequency * (vec[3] or 0)
	...
	local function pairAndLerp(ps, vs)
		...
		for p1: Vector, p2: Vector in pairs(pointPairs) do
...
function Simplex:Clone(): NoiseSolver
